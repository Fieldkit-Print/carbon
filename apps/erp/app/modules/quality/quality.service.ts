...
export async function getIssueActionTasks(
  ...
  let asanaMappings: Map<string, unknown> = new Map();

  if (taskIds.length > 0) {
    const [{ data: linearData }, { data: jiraData }, { data: asanaData }] =
      await Promise.all([
        client
    ...
    asanaMappings = new Map(
      (asanaData ?? []).map((m) => [m.entityId, m.metadata])
    );
  }
  ...
  return {
    ...
    data: result.data.map((task) => ({
      ...
      asanaTask: asanaMappings.get(task.id) ?? null
    }))
  };
}
