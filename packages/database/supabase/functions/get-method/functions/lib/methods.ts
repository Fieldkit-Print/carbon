export async function getJobMethodTree(client, methodId, parentMaterialId = null) {
...
export function getJobMethodTreeArray(client, methodId) {
  return client.rpc("get_job_methods_by_method_id", {
    mid: methodId
  });
}
function getJobMethodTreeArrayToTree(items, parentMaterialId = null) {
...
export function traverseJobMethod(node, callback) {
  callback(node);
  if (node.children) {
...
export async function getQuoteMethodTree(client, methodId, parentMaterialId = null) {
...
export function getQuoteMethodTreeArray(client, methodId) {
  return client.rpc("get_quote_methods_by_method_id", {
    mid: methodId
  });
}
function getQuoteMethodTreeArrayToTree(items, parentMaterialId = null) {
  const rootItems = [];
  const lookup = {};
  for (const item of items){
    const itemId = item.methodMaterialId;
...
export async function traverseQuoteMethod(node, callback) {
  await callback(node);
  if (node.children) {
...
export const getRatesFromWorkCenters = (workCenters)=>(processId, workCenterId)=>{
    if (!workCenters) {
      return {
        laborRate: 0,
        machineRate: 0,
    ...
    if (workCenterId) {
      const workCenter = workCenters?.find((wc)=>wc.id === workCenterId && wc.active);
      if (workCenter) {
        return {
    ...
    if (relatedWorkCenters.length > 0) {
      const laborRate = relatedWorkCenters.reduce((acc, workCenter)=>{
        return acc += workCenter.laborRate ?? 0;
      }, 0) / relatedWorkCenters.length;
      const machineRate = relatedWorkCenters.reduce((acc, workCenter)=>{
        return acc += workCenter.machineRate ?? 0;
      }, 0) / relatedWorkCenters.length;
      const overheadRate = relatedWorkCenters.reduce((acc, workCenter)=>{
...
export const getRatesFromSupplierProcesses = (processes, processPriceBreaks)=>(processId, supplierProcessId, quantity)=>{
    if (!processes) {
      return {
    ...
    if (supplierProcessId) {
      const supplierProcess = processes?.find((sp)=>sp.id === supplierProcessId);
      if (supplierProcess) {
        const breaks = processPriceBreaks?.[supplierProcessId] ?? [];
        const operationUnitCost = quantity && breaks.length > 0
    ...
    if (relatedProcesses.length > 0) {
      const operationMinimumCost = relatedProcesses.reduce((acc, process)=>{
        return acc += process.minimumCost ?? 0;
      }, 0) / relatedProcesses.length;
      const operationLeadTime = relatedProcesses.reduce((acc, process)=>{
...
async function getSupplierPriceBreaksForItems(client, itemIds) {
  if (!itemIds.length) return {};
  const supplierParts = await client.from("supplierPart").select("id, itemId, unitPrice").in("itemId", itemIds);
  if (!supplierParts.data?.length) return {};
  const supplierPartIds = supplierParts.data.map((sp)=>sp.id);
  const prices = await client.from("supplierPartPrice").select("supplierPartId, quantity, unitPrice").in("supplierPartId", supplierPartIds).order("quantity", {
    ascending: true
  });
...
function lookupPriceFromBreaks(priceBreaks, requestedQty, fallbackPrice) {
  ...
  if (eligible.length) {
    return eligible.reduce((best, pb)=>pb.quantity > best.quantity ? pb : best).unitPrice;
  }
...
function lookupBuyPriceFromMap(itemId, requestedQty, priceMap, fallbackCost) {
  ...
  return lookupPriceFromBreaks(entry.priceBreaks, requestedQty, entry.fallbackUnitPrice ?? fallbackCost);
}
...
export async function calculateQuoteLinePrices(client, quoteId, quoteLineId, companyId, userId) {
  // 1. Fetch data in parallel
  const [quoteLineResult, settingsResult, quoteResult, operationsResult] = await Promise.all([
    client.from("quoteLine").select("quantity, methodType, unitPricePrecision").eq("id", quoteLineId).single(),
    client.from("companySettings").select("quoteLineCategoryMarkups").eq("id", companyId).single(),
  ...
  const quantities = quoteLine.quantity ?? [
    1
  ];
  ...
  const rawMarkups = settingsResult.data.quoteLineCategoryMarkups ?? {};
  ...
  const buyMaterials = await client.from("quoteMaterial").select("id, itemId, unitCost").eq("quoteLineId", quoteLineId).eq("methodType", "Buy");
  const buyItemIds = [
  ...
  for (const mat of buyMaterials.data ?? []){
    ...
    if (price !== mat.unitCost) {
      await client.from("quoteMaterial").update({
        unitCost: price
      }).eq("id", mat.id);
    }
  }
  // 3. Build the quote method tree
  const rootMethod = await client.from("quoteMakeMethod").select("id").eq("quoteLineId", quoteLineId).is("parentMaterialId", null).single();
  ...
  function buildEnhancedTree(node, parentQuantity) {
    ...
    const nodeOps = operations.filter((o)=>o.quoteMakeMethodId === node.data.quoteMaterialMakeMethodId);
    return {
  ...
  function pushBuyCostEffect(itemId, itemType, quantity, unitCost) {
    const costFn = (outerQty)=>{
      const requestedQty = quantity * outerQty;
      const resolved = lookupBuyPriceFromMap(itemId, requestedQty, priceMap, unitCost);
  ...
  function walkTree(node) {
    if (node.methodType === "Buy") {
      ...
      const costFn = (quantity)=>node.unitCost * node.quantity * quantity;
    ...
    for (const operation of node.operations){
      if (operation.operationType === "Inside") {
        if (operation.setupTime) {
          const { fixedHours, hoursPerUnit } = normalizeTimeToHours(operation.setupTime, operation.setupUnit);
          effects.laborCost.push((quantity)=>{
            return hoursPerUnit * quantity * node.quantity * (operation.laborRate ?? 0) + fixedHours * (operation.laborRate ?? 0);
          });
          effects.overheadCost.push((quantity)=>{
            return hoursPerUnit * quantity * node.quantity * (operation.overheadRate ?? 0) + fixedHours * (operation.overheadRate ?? 0);
          });
        }
        ...
        if (operation.laborTime) {
          const normalized = normalizeTimeToHours(operation.laborTime, operation.laborUnit);
          laborFixedHours = normalized.fixedHours;
          laborHoursPerUnit = normalized.hoursPerUnit;
          effects.laborCost.push((quantity)=>{
            return laborHoursPerUnit * quantity * node.quantity * (operation.laborRate ?? 0) + laborFixedHours * (operation.laborRate ?? 0);
          });
        }
        if (operation.machineTime) {
          const normalized = normalizeTimeToHours(operation.machineTime, operation.machineUnit);
          machineFixedHours = normalized.fixedHours;
          machineHoursPerUnit = normalized.hoursPerUnit;
          effects.machineCost.push((quantity)=>{
            return machineHoursPerUnit * quantity * node.quantity * (operation.machineRate ?? 0) + machineFixedHours * (operation.machineRate ?? 0);
          });
        }
        ...
        effects.overheadCost.push((quantity)=>{
          if (hoursPerUnit * quantity * node.quantity > fixedHours) {
            return hoursPerUnit * quantity * node.quantity * (operation.overheadRate ?? 0);
          } else {
        ...
        effects.outsideCost.push((quantity)=>{
          const unitCost = operation.operationUnitCost * node.quantity * quantity;
  ...
  const priceRows = quantities.map((qty)=>{
    ...
    for (const key of costCategoryKeys){
      categoryCosts[key] = effects[key].reduce((acc, effect)=>acc + effect(qty), 0);
